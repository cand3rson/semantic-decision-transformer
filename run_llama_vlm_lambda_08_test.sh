...
################################################################################
# LLaMA VLM Lambda Sensitivity Test: λ=0.8
################################################################################
# 
# Purpose: Test LLaMA + VLM with HIGH text loss weight (λ=0.8)
#
# Comparison:
# - Baseline (λ=0.1): ADE=3.42m, FDE=7.25m, Miss=68.2%
# - Lambda=0.5: ADE=6.23m, FDE=13.45m, Miss=55.6%
# - Lambda=0.8: Testing now...
#
# Context: 
# - Decision Transformer λ=0.8: Testing separately
# - GPT-2 λ=0.8: ADE=6.86m (significant degradation)
# - LLaMA λ=0.8: Testing to complete architecture comparison
#
################################################################################

TIMESTAMP=$(date +%Y%m%d_%H%M%S)
LOG_FILE="training_llama_vlm_lambda08_${TIMESTAMP}.log"
...
echo "
================================================================================
LLaMA Lambda 0.8 test completed!
Log saved to: $LOG_FILE

Next steps:
1. Compare with λ=0.1 baseline (ADE=3.42m)
2. Compare with λ=0.5 test (ADE=6.23m)
3. Compare with Decision Transformer λ=0.8
4. Compare with GPT-2 λ=0.8 (ADE=6.86m)
5. Create comprehensive lambda sensitivity analysis across all architectures
================================================================================
"
