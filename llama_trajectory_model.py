#!/usr/bin/env python3
"""
LLaMA Trajectory Predictor Wrapper
===================================

This wrapper makes LLaMA compatible with the Decision Transformer interface
...
- LLaMA transformer backbone (instead of custom GPT)
...
class LlamaTrajectoryPredictor(TrajectoryModel):
    """
    LLaMA based trajectory predictor with Decision Transformer interface.
    
    This model uses LLaMA to model (Return_1, state_1, action_1, Return_2, state_2, ...)
    exactly like Decision Transformer, but with LLaMA as the backbone.
    """
    ...
    def forward(self, states, actions, rewards, returns_to_go, timesteps, attention_mask=None, return_agent_emb=False):
        ...
        if attention_mask is None:
            # attention mask for LLaMA: 1 if can be attended to, 0 if not
