#!/usr/bin/env python3
"""
Qwen Trajectory Predictor Wrapper
===================================

This wrapper makes Qwen compatible with the Decision Transformer interface
...
- Qwen transformer backbone (instead of custom GPT)
...
class QwenTrajectoryPredictor(TrajectoryModel):
    """
    Qwen based trajectory predictor with Decision Transformer interface.
    
    This model uses Qwen to model (Return_1, state_1, action_1, Return_2, state_2, ...)
    exactly like Decision Transformer, but with Qwen as the backbone.
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        # Create Qwen config
        # Note: Qwen2 uses Grouped Query Attention (GQA) by default
        # We need to ensure num_key_value_heads matches num_attention_heads for standard attention
        config = Qwen2Config(
            hidden_size=hidden_size,
            num_hidden_layers=n_layer,
            num_attention_heads=n_head,
            num_key_value_heads=n_head,  # Set equal to num_attention_heads for standard MHA (not GQA)
            intermediate_size=hidden_size * 4,
            max_position_embeddings=max_length * 3,  # RTG, state, action per timestep
            vocab_size=1,  # Not used for embeddings
            sliding_window=None,  # Disable sliding window attention
            use_sliding_window=False,  # Explicitly disable
            attn_implementation="eager",  # Use eager (standard) attention instead of SDPA
            **kwargs
        )
    ...
    def forward(self, states, actions, rewards, returns_to_go, timesteps, attention_mask=None, return_agent_emb=False):
        ...
        if attention_mask is None:
            # attention mask for Qwen: 1 if can be attended to, 0 if not
        ...
        # Pass through Qwen transformer
        # Qwen2 expects attention_mask in a specific format for causal attention
