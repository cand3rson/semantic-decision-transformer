...
################################################################################
# Qwen VLM Lambda Sensitivity Test: λ=0.5
################################################################################
# 
# Purpose: Test Qwen + VLM with MODERATE text loss weight (λ=0.5)
#
# Comparison:
# - Baseline (λ=0.1): ADE=3.36m, FDE=6.39m, Miss=65.0% (detailed prompt)
# - Lambda=0.5: Testing now...
#
# Context: 
# - Decision Transformer λ=0.5: ADE=2.17m (low sensitivity)
# - GPT-2 λ=0.5: ADE=5.48m (high sensitivity)
# - LLaMA λ=0.5: ADE=6.23m (high sensitivity)
# - Qwen λ=0.5: Testing to complete architecture comparison
#
################################################################################

TIMESTAMP=$(date +%Y%m%d_%H%M%S)
LOG_FILE="training_qwen_vlm_lambda05_${TIMESTAMP}.log"
...
echo "
================================================================================
Qwen Lambda 0.5 test completed!
...
5. Analyze lambda sensitivity across all architectures
================================================================================
"
