...
################################################################################
# DeepSeek VLM Lambda Sensitivity Test: λ=0.8
################################################################################
# 
# Purpose: Test DeepSeek + VLM with HIGH text loss weight (λ=0.8)
#
# Comparison:
# - Baseline (λ=0.1): ADE=3.52m, FDE=8.20m, Miss=69.3%
# - Lambda=0.5: ADE=6.36m, FDE=14.53m, Miss=55.6%
# - Lambda=0.8: Testing now...
#
# Context: 
# - Decision Transformer λ=0.8: ADE=2.35m (very low sensitivity)
# - DeepSeek λ=0.5: ADE=6.36m (high sensitivity)
# - LLaMA λ=0.8: ADE=7.24m (high sensitivity)
# - Qwen λ=0.8: ADE=10.21m (very high sensitivity)
# - GPT-2 λ=0.8: ADE=6.86m (very high sensitivity)
#
################################################################################

TIMESTAMP=$(date +%Y%m%d_%H%M%S)
LOG_FILE="training_deepseek_vlm_lambda08_${TIMESTAMP}.log"
...
echo "
================================================================================
DeepSeek Lambda 0.8 test completed!
Log saved to: $LOG_FILE

Next steps:
1. Compare with λ=0.1 baseline (ADE=3.52m)
2. Compare with λ=0.5 test (ADE=6.36m)
...
6. Compare with Qwen λ=0.8 (ADE=10.21m)
7. Create FINAL comprehensive lambda sensitivity analysis across ALL architectures
================================================================================
"
